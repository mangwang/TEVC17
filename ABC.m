function ABC( run_num, func_names, func_num, options, optionalArgs  )
% Framework for Artificial Bee Colony (ABC).
%   Parameters:
%   run_num            - Run number
%                       [positive scalar]
...
if ~isfield(optionalArgs, 'nNeighbours') || (isfield(optionalArgs, 'nNeighbours') && run_num > 1)
    ABC_core(run_num, func_names, func_num, options, optionalArgs);
end
